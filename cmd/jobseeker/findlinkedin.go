...
var findLinkedInCmd = &cobra.Command{
	...
	Short: "Search Bing for a LinkedIn profile by keywords and display it",
	Long: `Searches Bing for the first LinkedIn profile matching the given keywords,
...
func runFindLinkedIn(cmd *cobra.Command, args []string) {
	...
	// Phase 1: search Bing to find the LinkedIn profile URL.
	var profileURL, html string
	const maxSearchRetries = 5
	for attempt := 1; attempt <= maxSearchRetries; attempt++ {
		var err error
		if puppeteerClient != nil {
			profileURL, html, err = puppeteerClient.SearchLinkedIn(keywords)
		} else {
			profileURL, html, err = pool.SearchAndFetchLinkedIn(keywords)
		}
		if err != nil {
